...
(function ($) {
	...
	$.prettyDate = {
		...
		format: function (target) {
			...
			if(dayDiff > 0){
				...
				futuremessages = $.prettyDate.futuremessages;
				dayDiff = Math.abs(dayDiff);
				return dayDiff === 0 && (
						diff < 60 && futuremessages.now ||
						diff < 120 && futuremessages.minute ||
						diff < 3600 && futuremessages.minutes(Math.floor(diff / 60)) ||
						diff < 7200 && futuremessages.hour ||
						diff < 86400 && futuremessages.hours(Math.floor(diff / 3600))) ||
						dayDiff === 1 && futuremessages.tomorrow ||
						dayDiff < 7 && futuremessages.days(dayDiff) ||
						dayDiff < 8 && futuremessages.week ||
						dayDiff < 14 && futuremessages.days(dayDiff) ||
						dayDiff < 30 && futuremessages.weeks(Math.ceil(dayDiff / 7)) ||
						dayDiff < 34 && futuremessages.month ||
						dayDiff < 363 && futuremessages.months(Math.ceil(dayDiff / 31)) ||
						dayDiff <= 380 && futuremessages.year ||
						dayDiff > 380 && futuremessages.years(Math.ceil(dayDiff / 365));
			}
	...
	$.prettyDate.futuremessages = {
			now: 'just now',
			minute: 'in 1 minute',
			minutes: $.prettyDate.template('in {0} minutes'),
			hour: 'in 1 hour',
			hours: $.prettyDate.template('in {0} hours'),
			tomorrow: 'Tomorrow',
			days: $.prettyDate.template('in {0} days'),
			week: 'in 1 week',
			weeks: $.prettyDate.template('in {0} weeks'),
			month: 'in 1 month',
			months: $.prettyDate.template('in {0} months'),
			year: 'in 1 year',
			years: $.prettyDate.template('in {0} years')
	};
